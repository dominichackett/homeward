...
function decryptBiometricEmbedding(ciphertext) {
  ...
  try {
    const decoded = typeof atob === "function" ? atob(raw) : base64Decode2(raw);
...
var onHttpTrigger = async (runtime2, payload) => {
  runtime2.log("Chainlink CRE: Received confidential match request in TEE enclave.");
  try {
    runtime2.log("Raw payload keys: " + Object.keys(payload || {}).join(", "));
    runtime2.log("Raw payload preview: " + JSON.stringify(payload).slice(0, 200));
  } catch {}
  let body = null;
  try {
    ...
    runtime2.log("Error parsing trigger payload: " + String(err));
  }
  const finderEmbeddingCiphertext = body?.encrypted_embedding || "";
  const candidates = body?.candidates || [];
  const threshold = runtime2.config?.matchThreshold ?? 0.4;
  runtime2.log(`[TEE ENCLAVE] Ingested ${candidates.length} candidate embeddings.`);
  const finderVector = decryptBiometricEmbedding(finderEmbeddingCiphertext);
  if (!finderVector || finderVector.length !== 128) {
    runtime2.log("Finder biometric descriptor could not be decrypted or length is invalid.");
  ...
  runtime2.log(`[TEE ENCLAVE] Match evaluation complete. Matched: ${isMatch}, Distance: ${minDistance.toFixed(4)}`);
  return JSON.stringify({
    matched: isMatch,
    dependent_id: isMatch ? bestMatchId : null,
    euclidean_distance: isMatch ? Math.round(minDistance * 1e4) / 1e4 : null,
