...
export default function LoginPage() {
  ...
  // Sign In method: 'magic_link' | 'password'
  ...
  // 1. Submit Magic Link / OTP Sign-In
  const handleMagicLinkSignIn = async (e: React.FormEvent) => {
    ...
    if (!supabase) {
      ...
      localStorage.setItem(
        ...
        JSON.stringify({ email, id: "caregiver-" + Date.now().toString(36), role: "authenticated" })
      );
      setTimeout(() => {
        setIsLoading(false);
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (!supabase) {
      localStorage.setItem(
        ...
        JSON.stringify({
          ...
          id: "guardian-" + Date.now().toString(36),
          role: "authenticated",
        })
      );
      setTimeout(() => {
    ...
    try {
      ...
      if (data.session) {
        // Automatically logged in (confirm email disabled in Supabase project)
        router.push(redirectTo);
      } else if (data.user) {
        // Confirmation email dispatched
        setSignUpSuccessNotice(
          `Guardian account created! A confirmation link has been dispatched to ${email}. Please check your inbox to activate your account.`
        );
      }
    } catch (err: any) {
      setErrorMessage(err.message || "Failed to create guardian account. Please try again.");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <main className="max-w-md w-full mx-auto px-4 py-8">
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl animate-fade-in flex flex-col items-center text-center">
          ...
          <h1 className="text-2xl font-bold text-white tracking-tight">
            {activeTab === "signin" ? "Caregiver Sign In" : "Create Guardian Account"}
          </h1>
          <p className="text-xs text-slate-400 mt-1.5 leading-relaxed max-w-xs">
            {activeTab === "signin"
              ? "Access enrolled dependents, medical de-escalation notes, and emergency alert contacts."
              : "Register as an authorized caregiver or legal guardian to protect vulnerable loved ones."}
          </p>

          {/* Top Tabs: Sign In vs Create Account */}
          <div className="mt-5 w-full grid grid-cols-2 p-1 rounded-2xl bg-slate-950 border border-slate-800 text-xs font-semibold">
            <button
              type="button"
          ...
          {/* Sign Up Success Notice */}
          {signUpSuccessNotice && (
            <div className="mt-4 p-4 rounded-2xl bg-emerald-950/60 border border-emerald-800 text-emerald-200 text-xs text-left w-full space-y-2 animate-fade-in">
          ...
          {magicLinkSent && activeTab === "signin" && (
            <div className="mt-6 w-full p-5 rounded-2xl bg-slate-950/90 border border-emerald-500/40 text-left space-y-3 animate-fade-in">
              ...
              <p className="text-slate-300 text-xs leading-relaxed">
                ...
                Click the link in your email to unlock your Caregiver Dashboard.
              </p>
              <button
          ...
          {/* ==================== FORM BODY ==================== */}
          {!magicLinkSent && !signUpSuccessNotice && (
            <>
              {/* If in Sign In: Mode Toggle between Magic Link & Password */}
              {activeTab === "signin" && (
                <div className="mt-4 w-full grid grid-cols-2 p-1 rounded-xl bg-slate-950/60 border border-slate-800/80 text-[11px]">
                  <button
                    type="button"
              ...
              >
                ...
                {(activeTab === "signup" || authMode === "password") && (
                  <div>
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full mt-3 py-3 px-4 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-bold text-xs shadow-lg shadow-cyan-500/25 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                >
                  {isLoading ? (
          ...
          {/* Bottom Switcher */}
          <div className="mt-6 pt-4 border-t border-slate-800/80 w-full text-center text-xs text-slate-400">
            {activeTab === "signin" ? (
              <span>
                New caregiver or legal guardian?{" "}
                <button
                  type="button"
                  onClick={() => switchTab("signup")}
          ...
          <div className="mt-4 p-3 rounded-xl bg-slate-950/60 border border-slate-800/80 text-[11px] text-slate-400 text-left flex items-start gap-2">
            <HeartHandshake className="w-4 h-4 text-cyan-400 shrink-0 mt-0.5" />
            <span>
              Homeward uses Supabase Row-Level Security (RLS) to ensure only authenticated guardians can enroll or view sensitive dependent records.
            </span>
          </div>
        </div>
      </main>
